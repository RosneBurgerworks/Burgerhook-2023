...
static settings::Int crumb_tollerance("nav.crumb-tollerance", "45");
...
static settings::Int stuck_blacklist_time{ "nav.anti-stuck.blacklist-time", "10" };
static settings::Boolean path_during_setup{ "nav.path-during-setup", "true" };
...
static void followCrumbs()
{
    ...
    if (crumbs.size() > 0 && crumbs[0].vec.DistTo(g_pLocalPlayer->v_Origin) < *crumb_tollerance)
    {
        last_crumb = crumbs[0];
...
if (!(g_pLocalPlayer->holding_sniper_rifle && g_pLocalPlayer->bZoomed) && !(g_pLocalPlayer->bRevved || g_pLocalPlayer->bRevving) &&
    ((crouch || crumbs[0].vec.z - g_pLocalPlayer->v_Origin.z > 18) && last_jump.check(200)) ||
    (last_jump.check(200) && inactivity.check(*stuck_time / 2)))
{
...
// Look at path
    if (look && !hacks::aimbot::isAiming())
    {
        ...
        static int wait_time = 2;
        static int aim_speed = 16;
