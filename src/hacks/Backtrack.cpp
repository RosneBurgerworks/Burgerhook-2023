...
// Simplified getLatency function
float getLatency()
{
    INetChannel* ch = (INetChannel*)g_IEngine->GetNetChannelInfo();

    // If there's no net channel, return 0
    if (!ch)
        return 0.0f;

    // Calculate the real latency once
    float real_latency = ch->GetLatency(FLOW_OUTGOING) * 1000.0f;

    // Calculate and return the backtrack latency
    return latency_rampup * std::clamp(*latency, 0.0f, 900.0f - real_latency);
}
...
void MoveToTick(BacktrackData data)
{
   ...
   // Constants to represent bit positions
const int DIRTY_SPATIAL_PARTITION_BIT = 15;
const int DIRTY_SPATIAL_PARTITION_FLAG = 1 << DIRTY_SPATIAL_PARTITION_BIT;

// Mark for update
int* entity_flags = (int*)((uintptr_t)RAW_ENT(target) + 400);
*entity_flags |= DIRTY_SPATIAL_PARTITION_FLAG;
...
static void CreateMoveEarly()
{
    ...
    latency_rampup = std::min(0.0f, latency_rampup);

    int maxClients = g_IEngine->GetMaxClients();
    if ((int) bt_data.size() != maxClients)
        bt_data.resize(maxClients);
