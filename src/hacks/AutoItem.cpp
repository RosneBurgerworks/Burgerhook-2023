...
static settings::String secondary{ "auto-item.weapons.secondary", "642" };
static settings::String melee{ "auto-item.weapons.melee", "-1" };

static settings::Boolean hats{ "auto-item.hats", "false" };
static settings::Boolean hats_random{ "auto-item.hats.random", "false" };
...
static std::vector<int> achievement_hats{ 940, 302, 668, 774, 941, 744, 1164, 1169 };
static std::array<int, 3> random_hats;

void setupRandomHats()
{
    for (int i = 0; i < 3; ++i)
    {
        int idx        = UniformRandomInt(0, achievement_hats.size() - 1);
        random_hats[i] = achievement_hats[idx];

        // Erase that hat so we do not use it multiple times
        achievement_hats.erase(achievement_hats.begin() + idx);
    }
}
...
static void CreateMove()
{
    ...
    if (clazz != 0)
    {
        ...
        if (hats)
        {
            ...
            equipItem(clazz, slots[offset], *hats_random ? random_hats[0] : *hat1);
            equipItem(clazz, slots[(offset + 1) % 3], *hats_random ? random_hats[1] : *hat2);
            equipItem(clazz, slots[(offset + 2) % 3], *hats_random ? random_hats[2] : *hat3);
            offset = (offset + 1) % 3;
        }
