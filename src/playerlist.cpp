...
const std::string k_Names[]                                     = { "DEFAULT", "FRIEND", "RAGE", "IPC", "CAT", "PRIVATE", "PARTY" };
const char *const k_pszNames[]                                  = { "DEFAULT", "FRIEND", "RAGE", "IPC", "CAT", "PRIVATE", "PARTY" };
const std::array<std::pair<k_EState, size_t>, 4> k_arrGUIStates = { std::pair(k_EState::DEFAULT, 0), { k_EState::FRIEND, 1 }, { k_EState::RAGE, 2 } };
...
{
#if ENABLE_VISUALS
    return data.color || data.state == k_EState::FRIEND || data.state == k_EState::RAGE || data.state == k_EState::PRIVATE;
#endif
    return data.state == k_EState::FRIEND || data.state == k_EState::RAGE || data.state == k_EState::PRIVATE;
}
...
{
    if (entity->player_info.friendsID)
